...
class JackRentalMDP(MDP):
    """ Although (SB18) just pose this as a problem (see Exercise 4.7) I think it is pretty hard even with hints.
     So enter at your own peril :-).
     The overall strategy to solve the problem is similar to the Gambler environment: Just implement the relevant functions in a MDP-class,
     and then solve using value iteration (as we have already implemented). The problem occurs because the state includes
     the cars at each rental location, and the next-state distribution is tricky to compute due to the Poisson-distributions.
     """
