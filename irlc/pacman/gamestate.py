...
class GameState:
    ...
    > env = PacmanEnvironment() # This creates a new environment
    > state = env.reset()  # Call the reset-method as usual and return the initial state

    The tricky thing is that `state` is a GameState instance -- i.e. has all the methods found in
    *this* class. So for instance to know if the game is won or lost you can do:

    > print("Did we win?", state.isWin())

    Or to get the *next* state representing what occurs when you take an action `a`:

    > next_state = state.f(a)
